...
# PARAMETROS  ####################################
R0s <- read.csv("R0Estados11abril.csv")

#En el csv las columnas son
#1 = Nombre de estado

 #LLRR:  Yo he añadido la columna est con el codigo numérico. Modifique orden de estados para que fuera concordante con el orden de INEGI

#2 = Número total de casos confirmados
#3 y 4 = R0 con todos los datos
#5 y 6 = R0 con sólo los datos después del 15 de marzo (con intervención)
#7 y 8 = R0 con sólo la primera parte (antes del 16 de marzo sin intervención)
#Los ceros son casos en los que las rutinas del paquete no corrieron.
#Internamente el paquete selecciona un segmento de datos en donde detecta
#el crecimiento exponencial al inicio de epidemia, así que puede no haber
#diferencia entre los casos (3,4) con los (7,8).
#Finalmente, hay algunos casos en los que se reportan números extremadamente altos.
#Así que tomarlos con cautela (o probar más opciones del paquete que no he examinado)  --Rogelio Ramos


all(R0s[,8]==R0s[,9])

R0 <- R0s[,8]
plot(R0)  #hay valores iguales a 0, lo obviamente indica que no se presentaron casos antes de la fecha, pero dejar una R0 con este valor es erróneo. Les imputo la media de los valores sí observados
R0[R0==0]<-mean(R0[R0!=0])
...
bet<-R0*(gama1+gama1+gama3+gama4)/4
...
a <- cbind(sim2[,1] + 150,sim2[,-1])
b <- cbind(sim3[,1] + 300,sim2[,-1])
...
pdf(file =paste0("Total_Compartimento_450", ".pdf"))
...
matplot(sim[,1],rowSums(sim[,c(E1_i,E2_i,E3_i)]),t="l", col=1, xlab="Dias",ylab="Poblacion", main="Expuestos, GE 3")
...
matplot(sim[,1],rowSums(sim[,c(I1_4_i,I2_4_i,I3_4_i)]),t="l", col=3,xlab="Dias",ylab="Poblacion", main="Inf. CU")
...
matplot(sim[,1],rowSums(sim[,D_i]),t="l", col=5,xlab="Dias",ylab="Poblacion", main="Defunciones")

matplot(sim[,1],rowSums(sim[,R_i]),t="l", col=5,xlab="Dias",ylab="Poblacion", main="Removidos")

matplot(sim[,1],rowSums(sim[,Y1_i]),t="l", col=6,xlab="Dias",ylab="Poblacion", main="Acumulado de nuevos Infecciosos, GE 1")
matplot(sim[,1],rowSums(sim[,Y2_i]),t="l", col=6,xlab="Dias",ylab="Poblacion", main="Acumulado de nuevos Infecciosos, GE 2")
matplot(sim[,1],rowSums(sim[,Y3_i]),t="l", col=6,xlab="Dias",ylab="Poblacion", main="Acumulado de nuevos Infecciosos, GE 3")
matplot(sim[,1],rowSums(sim[,c(Y1_i,Y2_i,Y3_i)]),t="l", col=6,xlab="Dias",ylab="Poblacion", main="Acumulado de nuevos Infecciosos")
dev.off()
...
pdf(file = paste0("Compartimento_Estado_450", ".pdf"))
